...
void worstFit(struct Process processes[], int num_processes, int blocks[], int num_blocks) {
    ...
    for (int i = 0; i < num_processes; i++) {
        ...
        if (worst_index != -1) {
            ...
            blocks[worst_index] -= processes[i].memory_size;
        } else {
            allocation[i] = -1;
        }
    }
...
int main() {
    ...
    worstFit(processes, num_processes, blocks, num_blocks);
